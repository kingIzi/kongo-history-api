...
public class ComicService {
    ...
    public HttpDataResponse<Comic> createComic(final MultipartFile thumbnail, final MultipartFile data,final AddComicForm addComicForm){
        ...
        try{
            ...
            final var uploadedThumbnail = this.comicRepository.uploadFile(thumbnail).orElseThrow(AppUtilities.supplyException("Failed to upload comic thumbnail. Please contact support.",AppConst._KEY_CODE_INTERNAL_ERROR));
            addComicForm.setThumbnailFileName(uploadedThumbnail.getFirst());
            addComicForm.setThumbnailUrl(uploadedThumbnail.getSecond());
            final var uploadedData = this.comicRepository.uploadFile(data).orElseThrow(AppUtilities.supplyException("Failed to upload comic file. Please contact support.",AppConst._KEY_CODE_INTERNAL_ERROR));
            addComicForm.setDataFileName(uploadedData.getFirst());
            addComicForm.setDataUrl(uploadedData.getSecond());
            final var comic = this.comicRepository.save(this.makeComic(addComicForm)).orElseThrow(AppUtilities.supplyException("Failed to upload comic.",AppConst._KEY_CODE_INTERNAL_ERROR));
    ...
    public HttpDataResponse<Comic> updateComic(final String comicId,final MultipartFile thumbnail, final MultipartFile data, final UpdateComicForm updateComicForm){
        final var httpDataResponse = new HttpDataResponse<Comic>();
        try{
            final var comic = this.comicRepository.get(comicId).orElseThrow(AppUtilities.supplyException("Comic not found",AppConst._KEY_CODE_PARAMS_ERROR));
            if (thumbnail != null && !thumbnail.isEmpty()){
                final var uploadedComic = this.comicRepository.uploadFile(thumbnail).orElseThrow(AppUtilities.supplyException("Failed to upload thumbnail file. Please contact support.",AppConst._KEY_CODE_INTERNAL_ERROR));
                updateComicForm.setThumbnailFileName(uploadedComic.getFirst());
                updateComicForm.setThumbnailUrl(uploadedComic.getSecond());
            }
            if (data != null && !data.isEmpty()){
                final var uploadData = this.comicRepository.uploadFile(data).orElseThrow(AppUtilities.supplyException("Failed to upload comic file. Please contact support",AppConst._KEY_CODE_INTERNAL_ERROR));
            ...
            e.printStackTrace();
            UtilityFormatter.formatMessagesParamsError(httpDataResponse,"SOMETHING WENT WRONG",AppConst._KEY_CODE_INTERNAL_ERROR);
        }
        return httpDataResponse;
    }
    ...
    public HttpDataResponse<List<Comic>> getComicList(final Integer limit){
        ...
        try{
            final var comics = this.comicRepository.searchByCriteria(limit == null ? 100 : limit).orElseThrow(AppUtilities.supplyException("Failed to retrieve comics.",AppConst._KEY_CODE_INTERNAL_ERROR));
    ...
    public HttpDataResponse<List<Comic>> getComicList(final Integer limit,final FindComicForm findComicForm){
        final var httpDataResponse = new HttpDataResponse<List<Comic>>();
        try{
            final var comics = this.comicRepository.searchByCriteria(limit == null ? 100 : limit,findComicForm).orElseThrow(AppUtilities.supplyException("Failed to retrieve comics.",AppConst._KEY_CODE_INTERNAL_ERROR));
            httpDataResponse.setResponse(comics);
        }
        catch (ValueDataException e){
            e.printStackTrace();
            ...
            e.printStackTrace();
            UtilityFormatter.formatMessagesParamsError(httpDataResponse,"Something went wrong. Please contact support.",AppConst._KEY_CODE_INTERNAL_ERROR);
        }
        return httpDataResponse;
    }
    ...
    public HttpDataResponse<String> removeComic(final String comicId){
        ...
        try{
            final var comic = this.comicRepository.get(comicId).orElseThrow(AppUtilities.supplyException("Comic not found",AppConst._KEY_CODE_PARAMS_ERROR));
            if (!comic.getThumbnailFileName().isBlank())
                this.comicRepository.removeFile(comic.getThumbnailFileName());
            if (!comic.getDataFileName().isBlank())

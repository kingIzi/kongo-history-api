...
public class AuthorRepository extends AbstractFirestoreRepository<Author> {
    ...
    public Optional<List<Author>> searchByCriteria(final int limit,final FindAuthorForm findAuthorForm) throws ValueDataException,Exception{
        ...
        if (values.isEmpty())
            throw new ValueDataException("SOMETHING WENT WRONG! FindAuthorForm is empty",AppConst._KEY_CODE_PARAMS_ERROR);
        if (values.size() == 1){
            final var query = this.getCollectionReference().whereEqualTo(values.get(0).getKey(),values.get(0).getValue()).limit(limit);
